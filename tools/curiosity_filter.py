...
def _resonance_score(item: dict) -> int:
    """0-10 heuristic resonance score. Higher = more Em-shaped."""
    combined = f"{item.get('title', '')} {item.get('snippet', '')}".lower()
    score = sum(1 for kw in EM_RESONANT_KEYWORDS if kw in combined)
    return min(score, 10)


def heuristic_filter(items: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    for item in items:
        ...
        if _hard_blocked(combined):
            item["_filter_reason"] = "hard_block"
            dropped.append(item)
            continue
        score = _resonance_score(item)
...
def _model_score_items(items: list[dict]) -> list[dict]:
