...
Loads context from key memory files, asks Perplexity what needs
attention, then executes a structured action list via GitHub API.
...
This script is intentionally mechanical — it executes what the
reasoning layer decides. All judgment happens in the LLM call.
"""
...
def build_context():
    """Assemble context string from key memory files."""
    ts = now_utc().strftime('%Y-%m-%d %H:%M UTC')
    day = now_utc().strftime('%A')

    schedule  = read_file(SCHEDULE_FILE, 2000)
    status    = read_file(STATUS_FILE, 2000)
    diary     = read_file(DIARY_FILE, 2000)   # last 2000 chars = recent entries
...
def handle_diary_entry(content):
    ...
    with open(DIARY_FILE, 'a') as f:
        f.write(entry)
    print(f'[think] diary entry written')
    return True
...
def handle_update_status(mood):
    try:
        ...
        print(f'[think] status updated: {mood}')
        return True
    except Exception as e:
...
def write_auto_log(actions, did_act):
    ...
    if not did_act:
        ...
        for a in actions:
            ...
            if act == 'diary_entry':
                lines.append(f'- Wrote diary entry')
            elif act == 'queue_post':
                lines.append(f'- Queued post: "{a.get("text", "")[:60]}"')
            elif act == 'update_status':
                lines.append(f'- Updated status: "{a.get("mood", "")}"')
...
def main():
    ...
    content = llm(SYSTEM_PROMPT, context, max_tokens=800)
